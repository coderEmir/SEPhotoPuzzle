...
#define SE_NavigationBarH self.navigationController.navigationBar.frame.size.height
#define SE_IsFullScreen  [UIApplication sharedApplication].statusBarFrame.size.height > 20
#define SE_BottomStateBarH (SE_IsFullScreen ? 60 : 0)
...
- (void)initResource
{
    ...
    #warning iOS7AddStatusHeight
    self.contentView =  [[UIScrollView alloc] initWithFrame:CGRectMake((self.view.frame.size.width - contentSize.width)/2.0f, 49, contentSize.width,contentSize.height)];
...
#warning iOS7AddStatusHeight
    if (size_height >= (self.view.frame.size.height- 0 - 44 -34)) {
        size_height = self.view.frame.size.height- 0 - 44 - 34;
...
- (void)initData
{
    self.meituContentView.images = self.images;
...
{
#warning iOS7AddStatusHeight
    self.boardAndEditView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 44- SE_NavigationBarH - 33 - 45 - SE_BottomStateBarH, self.view.frame.size.width, 30)];
    [self.boardAndEditView setBackgroundColor:[UIColor clearColor]];
    [self.view addSubview:self.boardAndEditView];
    
    self.boardbutton = [[UIButton alloc] initWithFrame:CGRectMake(60, 0, 85, 30)];
    self.editbutton  = [[UIButton alloc] initWithFrame:CGRectMake(_boardAndEditView.frame.size.width-145, 0, 85, 30)];
    ...
    [self.boardbutton setBackgroundImage:[UIImage imageWithContentsOfFile:@"btn_meitu_board_normal" currentClass:self] forState:UIControlStateNormal];
    [self.boardbutton setBackgroundImage:[UIImage imageWithContentsOfFile:@"btn_meitu_board_highlight" currentClass:self] forState:UIControlStateHighlighted];
    [self.boardbutton setBackgroundImage:[UIImage imageWithContentsOfFile:@"btn_meitu_board_highlight" currentClass:self] forState:UIControlStateSelected];
    
    [self.editbutton setBackgroundImage:[UIImage imageWithContentsOfFile:@"btn_meitu_edit_normal" currentClass:self] forState:UIControlStateNormal];
    [self.editbutton setBackgroundImage:[UIImage imageWithContentsOfFile:@"btn_meitu_edit_highlight" currentClass:self] forState:UIControlStateHighlighted];
    [self.editbutton setBackgroundImage:[UIImage imageWithContentsOfFile:@"btn_meitu_edit_highlight" currentClass:self] forState:UIControlStateSelected];
    ...
    [self.boardbutton.titleLabel setFont:[UIFont systemFontOfSize:16.0f]];
    [self.editbutton.titleLabel setFont:[UIFont systemFontOfSize:16.0f]];
...
{
    #warning iOS7AddStatusHeight
    self.bottomControlView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 44 - 33 - 45 - SE_NavigationBarH - SE_BottomStateBarH, self.view.frame.size.width, 60)];
    ...
    #warning iOS7AddStatusHeight
    self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 44- SE_NavigationBarH - 33 - SE_BottomStateBarH, self.view.frame.size.width, 60)];
    ...
    [_storyboardButton setTitle:D_LocalizedCardString(@"card_meitu_storyboard") forState:UIControlStateNormal];
//    _posterButton = [[UIButton alloc] init];
//    [_posterButton setTitle:D_LocalizedCardString(@"card_meitu_poster")                   forState:UIControlStateNormal];
    _spliceButton = [[UIButton alloc] init];
    [_spliceButton setTitle:D_LocalizedCardString(@"card_meitu_splice")               forState:UIControlStateNormal];
...
- (void)controlButtonStyleSettingWithButton:(UIButton *)sender
{
    ...
    [sender.titleLabel setFont:[UIFont systemFontOfSize:16.f]];
...
- (void)showStoryboardAndPoster
{
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
        CGFloat bottomH = SE_IsFullScreen ? 28 : 0;
                         self.bottomControlView.frame =  CGRectMake(0, self.view.frame.size.height  - 33 - SE_NavigationBarH  - 50 - bottomH, self.view.frame.size.width, 50);
                         self.boardAndEditView.frame =  CGRectMake(0, self.view.frame.size.height  - 33 - 25 - SE_NavigationBarH - 60 - bottomH, self.view.frame.size.width, 30);
...
- (void)hiddenStoryboardAndPoster
{
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
        CGFloat bottomH = SE_IsFullScreen ? 28 : 0;
                         self.bottomControlView.frame =  CGRectMake(0, self.view.frame.size.height  - 33 - SE_NavigationBarH - bottomH, self.view.frame.size.width, 1);
                         self.boardAndEditView.frame =  CGRectMake(0, self.view.frame.size.height - 33 - 25 - SE_NavigationBarH - bottomH, self.view.frame.size.width, 30);
                     } completion:^(BOOL finished) {
...
{
    _storyboardView = [[GLStoryboardSelectView alloc] initWithFrame:CGRectMake(0, 0, self.bottomControlView.frame.size.width, self.bottomControlView.frame.size.height) picCount:[self.images count]];
...
- (void)didSelectedStoryboardPicCount:(NSInteger)picCount styleIndex:(NSInteger)styleIndex
{
    [[LoadingViewManager sharedInstance] showLoadingViewInView:self.view withText:@"正在处理"];
    dispatch_async(dispatch_get_main_queue(), ^{
        [self resetViewByStyleIndex:styleIndex imageCount:self.images.count];
...
- (void)spliceAction
{
    @synchronized(self){
        ...
        if (_spliceView.images == nil) {
            _spliceView.images = self.images;

            [[LoadingViewManager sharedInstance] showLoadingViewInView:self.view withText:@"正在处理"];
            dispatch_async(dispatch_get_main_queue(), ^{
                [self->_spliceView initData];
                [self.contentView setContentOffset:CGPointMake(self->_contentView.bounds.size.width*2, 0) animated:YES];
                [[LoadingViewManager sharedInstance] removeLoadingView:self.view];
...
- (void)showBoardButton
{
    ...
    _editbutton.frame  = CGRectMake(_boardAndEditView.frame.size.width-145, 0, 85, 30);
