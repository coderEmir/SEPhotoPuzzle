...
- (void)setStyleIndex:(NSInteger)index
{
    ...
    if (![picCountFlag isEqualToString:@""]) {
        ...
        _styleFileName = [NSString stringWithFormat:@"SEPhotoPuzzle.bundle/Plists/number_%@_style_%zd.plist",picCountFlag,_styleIndex];
        _styleDict = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle bundleForClass:self.class].resourcePath stringByAppendingPathComponent:_styleFileName]];
...
//                btn.center = originPoint;
                if (self->_tempView) {
                    [self exchangeFromIndex:btn.tag-1 toIndex:self->_tempView.tag-1];
                }else{
...
//    [_assets replaceObjectAtIndex:toIndex withObject:a];
    ...
    UIImage *a = [_images objectAtIndex:fIndex];
    UIImage *b = [_images objectAtIndex:toIndex];
    
    [_images replaceObjectAtIndex:fIndex withObject:b];
    [_images replaceObjectAtIndex:toIndex withObject:a];
